...
const getNetlifyFunctionName = (filePath) => {
  ...
  // Allow the function name of an api route to end in "-background"
  const isApiRoute = functionName.startsWith("next_api_");
  functionName = isApiRoute ? functionName.replace(/[^\w\d-]|-(?!background$)/g, "") : functionName.replace(/[^\w\d]/g, "");
